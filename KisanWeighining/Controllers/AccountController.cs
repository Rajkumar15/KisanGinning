...
namespace KisanWeighining.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task RemoveUserusingemail(string email)
        {
            var currentUser = UserManager.FindByName(email);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                   // await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);
                    ...
                    TempData["userid"] = currentUser.Id.ToString();
                    TempData["Username"] = currentUser.UserName.ToString();
                    return RedirectToAction("InfoRegister", "SystemMaster", new { userid = 0 });
                }
                AddErrors(result);
            }
        ...
        //get start Role page
        [HttpGet]
        public ActionResult RoleMaster(string _id)
        {
          
            if (_id != null)
            {
                var roleManager = new RoleManager<Microsoft.AspNet.Identity.EntityFramework.IdentityRole>(new RoleStore<IdentityRole>(new ApplicationDbContext()));             
                return View(roleManager.Roles.Where(x => x.Id == _id).FirstOrDefault());
            }
            else
            {
                return View();
        ...
        [HttpPost]
        public ActionResult RoleMaster(IdentityRole model)
        {            
            var roleManager = new RoleManager<Microsoft.AspNet.Identity.EntityFramework.IdentityRole>(new RoleStore<IdentityRole>(new ApplicationDbContext()));
            if (!roleManager.RoleExists(model.Name))
            {
                ...
                return RedirectToAction("RoleMaster", "Account");
            }
            else
            {
        ...
        public ActionResult GetRoleList()
        {
            ...
            try
            {
                var v = roleManager.Roles.ToList();
                if (!string.IsNullOrEmpty(search) && !string.IsNullOrWhiteSpace(search))
                {
                    v = roleManager.Roles.Where(x => x.Name.ToLower().Contains(search.ToLower())).ToList();
                }
